...
export default class State {
  ...
  static same(coordinate1, coordinate2) {
    return (
      coordinate1.length == coordinate2.length && !coordinate1.some((element, index) => element !== coordinate2[index])
    );
  }
  ...
  static generateState(rowCount, columnCount, mineCount, baseCoordinates) {
    ...
    const countNeighborMines = (y, x) => {
      return State.calculateNeighborCoordinates(y, x).filter((coordinate) =>
        State.contains(mineCoordinates, coordinate),
      ).length;
    };
